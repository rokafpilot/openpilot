...
def state_control(frame, rcv_frame, plan, path_plan, CS, CP, state, events, v_cruise_kph, v_cruise_kph_last,
  ...
  #Bolt EV cannot LongControl wihout pedal.
  # actuators.gas = 0.1
  # actuators.brake = 0.1
  ...
  if (lac_log.saturated and not CS.steeringPressed) or (saturated_count > STEER_ANGLE_SATURATION_TIMEOUT):
    ...
    left_deviation = actuators.steer > 0 and path_plan.dPoly[3] > 0.125
    right_deviation = actuators.steer < 0 and path_plan.dPoly[3] < -0.125
...
def data_send(sm, pm, CS, CI, CP, VM, state, events, actuators, v_cruise_kph, rk, AM,
  ...
  if len(md.meta.desirePrediction):
    ...
    l_lane_close = left_lane_visible and (sm['pathPlan'].lPoly[3] < (1.08 - cameraOffset))
    r_lane_close = right_lane_visible and (sm['pathPlan'].rPoly[3] > -(1.08 + cameraOffset))
